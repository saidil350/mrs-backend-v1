import { getDb } from "@/lib/db";
import type { Industry } from "@/types";

function mapRow(row: Record<string, unknown>): Industry {
  return {
    id: String(row.id),
    name: String(row.name),
    icon: row.icon ? String(row.icon) : null,
    imageId: row.image_id ? String(row.image_id) : null,
    description: String(row.description),
    expertiseSummary: row.expertise_summary ? String(row.expertise_summary) : null,
    applications: Array.isArray(row.applications) ? row.applications.map(String) : [],
    isPublished: Boolean(row.is_published),
    sortOrder: Number(row.sort_order),
    createdAt: new Date(String(row.created_at)).toISOString(),
    updatedAt: new Date(String(row.updated_at)).toISOString(),
  };
}

export async function getAllIndustries() {
  const result = await getDb().query(
    "SELECT * FROM industries ORDER BY sort_order, name",
  );
  return result.rows.map(mapRow);
}

export async function getIndustryById(id: string) {
  const result = await getDb().query(
    "SELECT * FROM industries WHERE id = $1",
    [id],
  );
  return result.rows[0] ? mapRow(result.rows[0]) : null;
}

export async function createIndustry(data: {
  name: string;
  icon?: string | null;
  imageId?: string | null;
  description: string;
  expertiseSummary?: string | null;
  applications?: string[];
  sortOrder?: number;
  isPublished?: boolean;
}) {
  const result = await getDb().query(
    `INSERT INTO industries (name, icon, image_id, description, expertise_summary, applications, sort_order, is_published)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING *`,
    [
      data.name,
      data.icon ?? null,
      data.imageId ?? null,
      data.description,
      data.expertiseSummary ?? null,
      data.applications ?? [],
      data.sortOrder ?? 0,
      data.isPublished ?? false,
    ],
  );
  return mapRow(result.rows[0]);
}

export async function updateIndustry(
  id: string,
  data: {
    name?: string;
    icon?: string | null;
    imageId?: string | null;
    description?: string;
    expertiseSummary?: string | null;
    applications?: string[];
    sortOrder?: number;
    isPublished?: boolean;
  },
) {
  const fields: string[] = [];
  const values: unknown[] = [];
  let idx = 1;

  if (data.name !== undefined) {
    fields.push(`name = $${idx++}`);
    values.push(data.name);
  }
  if (data.icon !== undefined) {
    fields.push(`icon = $${idx++}`);
    values.push(data.icon);
  }
  if (data.imageId !== undefined) {
    fields.push(`image_id = $${idx++}`);
    values.push(data.imageId);
  }
  if (data.description !== undefined) {
    fields.push(`description = $${idx++}`);
    values.push(data.description);
  }
  if (data.expertiseSummary !== undefined) {
    fields.push(`expertise_summary = $${idx++}`);
    values.push(data.expertiseSummary);
  }
  if (data.applications !== undefined) {
    fields.push(`applications = $${idx++}`);
    values.push(data.applications);
  }
  if (data.sortOrder !== undefined) {
    fields.push(`sort_order = $${idx++}`);
    values.push(data.sortOrder);
  }
  if (data.isPublished !== undefined) {
    fields.push(`is_published = $${idx++}`);
    values.push(data.isPublished);
  }

  if (fields.length === 0) return getIndustryById(id);

  fields.push(`updated_at = now()`);
  values.push(id);

  const result = await getDb().query(
    `UPDATE industries SET ${fields.join(", ")} WHERE id = $${idx} RETURNING *`,
    values,
  );
  return result.rows[0] ? mapRow(result.rows[0]) : null;
}

export async function deleteIndustry(id: string) {
  const result = await getDb().query(
    "DELETE FROM industries WHERE id = $1 RETURNING id",
    [id],
  );
  return result.rows[0]?.id ?? null;
}

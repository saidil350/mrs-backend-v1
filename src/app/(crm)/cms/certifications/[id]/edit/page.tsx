import { redirect, notFound } from "next/navigation";
import { getCurrentUser } from "@/lib/session";
import { getCertificationById } from "@/lib/cms/certifications";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";
import { EditCertificationForm } from "./edit-form";

export default async function EditCertificationPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");

  const { id } = await params;
  const certification = await getCertificationById(id);
  if (!certification) notFound();

  return (
    <div className="p-6 space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" render={<Link href="/cms/certifications" />}>
          <ArrowLeft className="size-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">Edit Sertifikasi</h1>
          <p className="text-muted-foreground">{certification.name}</p>
        </div>
      </div>

      <EditCertificationForm certification={certification} />
    </div>
  );
}
